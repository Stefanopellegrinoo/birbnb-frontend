...
console.log(payload)
  ...
  return (
    <Box sx={{ minHeight: 500 }}>
      <Card shadow="sm" padding="lg" mih={500} > {/* altura fija */}
        <Stepper
          active={active}
        ...
        >
          <Stepper.Step label="Datos básicos">
            <Stack sx={{ minHeight: 300, justifyContent: 'space-between' }}>
              <TextInput
                label="Nombre"
              ...
              <Group grow>
                <TextInput
                  ...
                  onChange={(e) => handleChange('horarioChkIn')(e.currentTarget.value)}
                />
                <TextInput
                  label="Check-out"
                  value={formData.horarioChkOut}
                  onChange={(e) => handleChange('horarioChkOut')(e.currentTarget.value)}
                />
              </Group>
          ...
          <Stepper.Step label="Extras">
            ...
            data={['wifi', 'estacionamiento', 'mascotas_permitidas', 'piscina']}
            ...
            <Stack  spacing="xs">
              ...
              {Object.entries(formData).map(([key, val]) => (
                <Group key={key} noWrap>
                  <Text size="sm" style={{ flex: 1, textTransform: 'capitalize' }}>{key.replace(/([A-Z])/g, ' $1')}:</Text>
                  <Text size="sm" style={{ flex: 2 }}>{String(val)}</Text>
                </Group>
              ))}
            </Stack>
          </Stepper.Step>
